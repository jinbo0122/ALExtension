...
- (int)busyRetryTimeout {
  ...
  DDLogError(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
  return -1;
}

- (void)setBusyRetryTimeout:(int)i {
  DDLogError(@"%s:%d", __FUNCTION__, __LINE__);
  DDLogError(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
  ...
  if (errmsg && [self logsErrors]) {
    DDLogError(@"Error inserting batch: %s", errmsg);
    sqlite3_free(errmsg);
  }
