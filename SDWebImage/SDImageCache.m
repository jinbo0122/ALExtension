...
{
  if (!image || !key || image.size.width<20)
  {
    return;
  }
//存储image时去掉query string
  NSArray *stringArray = [key componentsSeparatedByString:@"?"];
  if ([stringArray count]>1){
    key = @"";
    for (NSInteger i=0; i<[stringArray count]-1; i++) {
      key = [NSString stringWithFormat:@"%@%@",key,[stringArray safeObjectAtIndex:i]];
    }
  }
