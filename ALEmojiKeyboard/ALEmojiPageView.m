...
@implementation ALEmojiPageView
...
- (void)setButtonTexts:(NSMutableArray *)buttonTexts {
  ...
  if (([self.buttons count] - 1) == [buttonTexts count]) {
    ...
    for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
      if ([NSString stringContainsEmoji:buttonTexts[i]]) {
        [self.buttons[i] setTitle:buttonTexts[i] forState:UIControlStateNormal];
      }
      else{
        [self.buttons[i] setImage:[UIImage imageNamed:buttonTexts[i]] forState:UIControlStateNormal];
      }
      [self.buttons[i] setTagString:buttonTexts[i]];
    }
  }
  else {
    ...
    for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
      ...
      if ([NSString stringContainsEmoji:buttonTexts[i]]) {
        ...
        NSString *path = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"baobao_kb" ofType:@"bundle"]] pathForResource:[buttonTexts[i] stringByAppendingString:@"_kb"] ofType:@"png"];
        
        UIImage *image = [UIImage imageWithContentsOfFile:path];
        [button setImage:image forState:UIControlStateNormal];
      }
      [button setTagString:buttonTexts[i]];
...
- (void)emojiButtonPressed:(UIButton *)button {
  if (button.tag == BACKSPACE_BUTTON_TAG) {
    DDLogInfo(@"Back space pressed");
    [self.delegate emojiPageViewDidPressBackSpace:self];
    return;
  }
  DDLogInfo(@"%@", button.titleLabel.text);
  [self.delegate emojiPageView:self didUseEmoji:button.tagString];
}
